...
/**
 * Keeps the follow-ball viewport on the rendered pitch. Without this clamp,
 * following an event near a touchline can expose an empty canvas edge.
 */
export function resolveCameraFocus(
	...
	return [
		Math.max(
			halfVisibleWidth,
			Math.min(width - halfVisibleWidth, ball.x * width),
		),
		Math.max(
			halfVisibleHeight,
			Math.min(height - halfVisibleHeight, ball.y * height),
		),
	];
}
...
function drawPitch(
	...
	for (let index = 0; index < 10; index += 1) {
		context.fillStyle =
			index % 2 === 0 ? "rgba(255,255,255,0.035)" : "rgba(0,0,0,0.025)";
	...
	for (const direction of [1, -1]) {
		const spotX =
			direction === 1
				? marginX + pitchWidth * 0.11
				: width - marginX - pitchWidth * 0.11;
		context.beginPath();
		context.arc(
			spotX,
			height / 2,
		...
		context.strokeRect(
			direction === 1 ? marginX - goalDepth : width - marginX,
			height * 0.43,
			goalDepth,
			height * 0.14,
		);
	...
	for (const direction of [1, -1]) {
		const left =
			direction === 1 ? marginX : width - marginX - pitchWidth * 0.16;
		context.strokeRect(left, height * 0.24, pitchWidth * 0.16, height * 0.52);
		const sixLeft =
			direction === 1 ? marginX : width - marginX - pitchWidth * 0.07;
		context.strokeRect(
			sixLeft,
			height * 0.36,
			pitchWidth * 0.07,
	...
	for (const [x, y, start] of [
		[marginX, marginY, 0],
		[width - marginX, marginY, Math.PI / 2],
		[width - marginX, height - marginY, Math.PI],
		[marginX, height - marginY, Math.PI * 1.5],
	] as const) {
		context.beginPath();
...
function drawPlayer(
	...
	context.strokeStyle =
		presentationPlayer.side === "Home" ? "#ffffff" : "#111827";
	context.lineWidth = 2;
	context.beginPath();
	...
	if (showNames) {
		...
		context.fillText(
			names[names.length - 1] ?? presentationPlayer.player.name,
			x,
			y - radius - 8,
		);
	}
	if (showRoleLabels) {
		...
		context.fillStyle = "#f8fafc";
		context.fillText(
			roleAbbreviation(presentationPlayer.player.role),
			x,
			y + radius + 9,
		);
	}
...
function drawFormationShape(
	context: CanvasRenderingContext2D,
	players: readonly PresentationPlayer[],
	width: number,
	height: number,
	homeColor: string,
	awayColor: string,
	...
	for (const side of ["Home", "Away"] as const) {
		const team = players.filter(
			(player) => player.side === side && !player.goalkeeper,
		);
		context.save();
		...
		team.forEach((player, index) => {
			const nearest = team
				.slice(index + 1)
				.reduce<PresentationPlayer | null>((candidate, other) => {
					if (!candidate) return other;
					const currentDistance =
						(candidate.point.x - player.point.x) ** 2 +
						(candidate.point.y - player.point.y) ** 2;
					const nextDistance =
						(other.point.x - player.point.x) ** 2 +
						(other.point.y - player.point.y) ** 2;
...
function drawDebugPitchGuides(
	...
	for (
		let column = 1;
		column < MATCH_2D_CONFIG.debug.gridColumns;
		column += 1
	) {
...
function drawDebugDiagnostics(
	...
	context.fillRect(
		inset,
		height - MATCH_2D_CONFIG.debug.panelHeight - inset,
		Math.min(MATCH_2D_CONFIG.debug.panelWidth, width - inset * 2),
		MATCH_2D_CONFIG.debug.panelHeight,
	);
	context.fillStyle = "#f8fafc";
	...
	lines.forEach((line, index) => {
		context.fillText(
			line,
			inset + 8,
			height - MATCH_2D_CONFIG.debug.panelHeight + 18 + index * 20,
		);
	});
	context.restore();
}
...
export default function Match2DRenderer({
	...
	useEffect(() => {
		...
		const render = (now: number) => {
			...
			const boundedZoom = Math.max(
				MATCH_2D_CONFIG.camera.minZoom,
				Math.min(MATCH_2D_CONFIG.camera.maxZoom, zoom),
			);
			const activeZoom =
				cameraMode === "follow-ball"
					? Math.max(boundedZoom, MATCH_2D_CONFIG.camera.dynamicZoom)
					: boundedZoom;
			const [focusX, focusY] =
				cameraMode === "follow-ball"
					? resolveCameraFocus(
			...
			context.clip();
			context.translate(
				pitchViewport.x + pitchViewport.width / 2,
				pitchViewport.y + pitchViewport.height / 2,
			);
			...
			frame.players.forEach((player) => {
				const fallbackNumber =
					player.side === "Home" ? ++homeMarkerNumber : ++awayMarkerNumber;
				...
				drawPlayer(
					...
					player.id === frame.actorPlayerId ||
						player.id === frame.targetPlayerId,
					String(playerNumbers?.[player.id] ?? fallbackNumber),
					status.yellowCards,
					status.injured,
				);
			});
			if (showFormationShape || cameraMode === "tactical")
				drawFormationShape(
					context,
					frame.players,
					pitchViewport.width,
			...
			const [ballX, ballY] = toCanvas(
				frame.ball,
				pitchViewport.width,
				pitchViewport.height,
			);
			if (frame.activeClip && frame.ballTrajectory !== "ground") {
				const [fromX, fromY] = toCanvas(
					frame.activeClip.ballFrom,
					pitchViewport.width,
					pitchViewport.height,
				);
				context.save();
				context.strokeStyle =
					frame.ballTrajectory === "shot"
						? "rgba(250,204,21,0.8)"
				...
				context.quadraticCurveTo(
					(fromX + ballX) / 2,
					Math.min(fromY, ballY) - pitchViewport.height * 0.12,
					ballX,
					ballY,
				);
				context.stroke();
			...
			const ballRadius = Math.max(
				4,
				Math.min(pitchViewport.width, pitchViewport.height) * 0.011,
			);
			const ballStyle =
				frame.ballState === "save"
					? "#fb923c"
					: frame.ballState === "penalty"
						? "#facc15"
						: frame.ballState === "restart"
							? "#93c5fd"
							: frame.ballState === "loose"
								? "#cbd5e1"
								: "#ffffff";
			...
			context.strokeStyle =
				frame.ballState === "penalty" ? "#7c2d12" : "#111827";
			context.lineWidth = 1.5;
			context.beginPath();
			...
			if (frame.activeClip && shouldShowEventOverlay(frame.activeClip.event)) {
				...
				context.fillRect(
					12,
					12,
					Math.max(150, context.measureText(label).width + 24),
					28,
				);
				context.fillStyle = "#f8fafc";
	...
	return (
		<canvas
			...
			className="block h-full min-h-80 w-full bg-slate-950"
		/>
	);
}
