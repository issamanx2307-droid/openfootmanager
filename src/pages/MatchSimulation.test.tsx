...
vi.mock("../components/match/MatchLive", () => ({
  default: ({
    ...
    <div data-testid="match-live-container" data-preferred-speed={preferredSpeed ?? "normal"}>
      {snapshot.home_team.name}
      <button type="button" data-testid="match-live" onClick={onFullTime}>Full Time</button>
      <button type="button" data-testid="match-trigger-halftime" onClick={onHalfTime}>Half Time</button>
      <button type="button" data-testid="match-trigger-speed-fast" onClick={() => onPreferredSpeedChange?.("fast")}>Fast</button>
    </div>
  ),
}));
...
vi.mock("../components/match/HalfTimeBreak", () => ({
  default: ({ onResume }: { onResume?: () => void }) => (
    <div data-testid="halftime">
      <button type="button" data-testid="halftime-resume" onClick={onResume}>Resume</button>
    </div>
  ),
}));
