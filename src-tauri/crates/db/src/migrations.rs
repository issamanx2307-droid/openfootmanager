...
pub const MIGRATION_COUNT: usize = 47;
...
pub fn all_migrations() -> Migrations<'static> {
    Migrations::new(vec![
        ...
        M::up(include_str!("sql/v044_game_ruleset_metadata.sql")),
        // V45: Persist the ruleset-owned transfer calendar with each career.
        M::up(include_str!("sql/v045_game_transfer_windows.sql")),
        // V46: Persist manager-specific scouting knowledge and shortlist.
        M::up(include_str!("sql/v046_manager_scouting_knowledge.sql")),
        // V47: Persist authoritative server session metadata for reconnect/restart.
        M::up(include_str!("sql/v047_authoritative_session.sql")),
    ])
}
...
mod tests {
    ...
    fn test_apply_migrations_to_empty_db() {
        ...
        assert!(
            game_meta_columns.contains(&"ruleset_id".to_string()),
            "missing game_meta.ruleset_id"
        );
        assert!(
            game_meta_columns.contains(&"ruleset_version".to_string()),
            "missing game_meta.ruleset_version"
        );
        assert!(
            game_meta_columns.contains(&"transfer_windows_json".to_string()),
            "missing game_meta.transfer_windows_json"
        );
