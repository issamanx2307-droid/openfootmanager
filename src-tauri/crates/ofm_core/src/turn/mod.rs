    ...
    F: FnMut(StatsState),
{
    ...
    // Knockout progression can add new future fixtures after the initial
    // calendar build. Re-run the cross-competition check so a club never has
    // two scheduled matches on the same date.
